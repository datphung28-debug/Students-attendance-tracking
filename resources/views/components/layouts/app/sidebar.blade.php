    ...
    <flux:sidebar sticky stashable class="border-r border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <flux:navlist variant="outline">
            <flux:navlist.group :heading="__('Platform')" class="grid">
                <flux:navlist.item icon="home"
                    ...
                    wire:navigate>{{ __('Thông tin') }}</flux:navlist.item>
            </flux:navlist.group>
            @if (auth()->user()->role == 'admin')
            <flux:navlist.item icon="academic-cap" :href="route('student.index')"
                :current="request()->routeIs('student.index')" wire:navigate>{{ __('Quản lý sinh viên') }}
            </flux:navlist.item>
            <flux:navlist.item icon="bars-3-bottom-left" :href="route('grade.index')"
                :current="request()->routeIs('grade.index')" wire:navigate>{{ __('Quản lý lớp') }}
            </flux:navlist.item>
            @endif
            <flux:navlist.item icon="calendar-days" :href="route('attendance.page')"
                :current="request()->routeIs('attendance.page')" wire:navigate>{{ __('Quản lý điểm danh') }}
            </flux:navlist.item>
        </flux:navlist>
        ...
        <flux:navlist variant="outline">
            <flux:navlist.item icon="folder-git-2"
                href="https://github.com/datphung28-debug/Attendance-tracking-app.git" target="_blank">
                {{ __('Repository') }}
            </flux:navlist.item>
